...
import { Loader2, Plus, X, ArrowLeft, Sparkles, CheckCircle, RefreshCw } from 'lucide-react'
...
export default function EditJobPage() {
  ...
  const [form, setForm] = useState({
    ...
    status: 'active' as 'draft' | 'active' | 'paused' | 'closed',
  })
  ...
  useEffect(() => {
    if (jobData && jobData.title) {
      setForm({
        ...
        roleLevel: jobData.roleLevel || 'mid',
        experienceRange: jobData.experienceRange || { min: 0, max: 5 },
        salaryRange: jobData.salaryRange || { min: 0, max: 0, currency: 'USD' },
  ...
  const extract = useMutation({
    ...
    onSuccess: (data: AIPreview) => {
      ...
      toast.success('AI re-extracted job entity')
    },
  ...
  const update = useMutation({
    mutationFn: (reDecompose: boolean) =>
      fetch(`/api/jobs/${jobId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...form, reDecompose }),
      }).then((r) => {
    ...
    onError: (e: string) => toast.error(e || 'Failed to update job'),
  })
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-3xl px-4 py-10">
        <Link href={`/recruiter/jobs/${jobId}/pipeline`} className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mb-6 transition-colors">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={update.isPending || !form.title}
              className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-medium hover:bg-white/10 disabled:opacity-50 transition-colors"
            >
            ...
            <button
              ...
              disabled={update.isPending || !form.title}
              className="flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-violet-700 disabled:opacity-50 transition-colors"
              title="Saves changes and re-runs AI decomposition + regenerates embedding"
            >
        ...
        <div className="glass rounded-2xl p-6 mb-6 gradient-border space-y-4">
          ...
          <div>
            ...
            <div className="mt-2 flex items-center justify-between">
              <p className="text-xs text-muted-foreground">Use &quot;Save &amp; Re-index&quot; to regenerate AI skills and embedding.</p>
              <button
                type="button"
              ...
              >
                ...
                {extract.isPending ? 'Extracting…' : 'AI Preview'}
              </button>
            </div>
          </div>
            ...
            <div className="rounded-xl border border-violet-500/30 bg-violet-500/8 p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-violet-300">AI Re-extracted Entity</span>
                </div>
                {!previewApplied ? (
                  <button onClick={applyPreview} className="flex items-center gap-1.5 rounded-lg bg-violet-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-violet-700 transition-colors">
                    <CheckCircle className="h-3 w-3" /> Apply
                  </button>
                ) : (
                  <span className="flex items-center gap-1 text-xs text-green-400"><CheckCircle className="h-3 w-3" /> Applied</span>
                )}
              </div>
              <div className="space-y-1.5 text-xs">
                {aiPreview.requiredSkills?.length > 0 && (
                  <div><span className="text-muted-foreground">Skills: </span>{aiPreview.requiredSkills.map((s) => s.skill).join(', ')}</div>
                )}
                {aiPreview.experienceRange && (
                  <div><span className="text-muted-foreground">Experience: </span>{aiPreview.experienceRange.min}–{aiPreview.experienceRange.max} yrs</div>
                )}
                {aiPreview.roleLevel && (
                  <div><span className="text-muted-foreground">Level: </span><span className="capitalize">{aiPreview.roleLevel}</span></div>
                )}
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-xl border border-white/10 bg-[hsl(0,0%,6%)] px-4 py-3 text-sm focus:outline-none"
              >
        ...
        <div className="glass rounded-2xl p-6 mb-6 gradient-border">
          ...
          <div className="space-y-2">
            ...
            {form.requiredSkills.length === 0 && (
              <p className="text-sm text-muted-foreground">No skills. Use &quot;Save &amp; Re-index&quot; to auto-extract from description.</p>
            )}
          </div>
        </div>
          ...
          <div className="flex gap-3 flex-wrap">
            {(['draft', 'active', 'paused', 'closed'] as const).map((s) => (
              <button
                ...
                className={`flex-1 rounded-xl border py-2.5 text-sm font-medium capitalize min-w-[100px] transition-all ${
                  form.status === s
